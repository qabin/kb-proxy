...
import com.bin.kong.proxy.contract.request.user.LoginRequest;
...
public class LoginController {
    ...
    public GenericResponse ajax_user_login(@RequestBody LoginRequest request) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(request.getLogin_name())) {
                response.setStatus(ResponseConstants.FAIL_CODE);
                response.setMessage("请输入账号！");
                return response;
            }

            UserInfo user = userInfoMapper.selectByLoginName(request.getLogin_name().trim());
            ...
            UsernamePasswordToken token = new UsernamePasswordToken(request.getLogin_name().trim(), DigestUtils.md5DigestAsHex(request.getLogin_pwd().trim().getBytes()));
            subject.login(token);
