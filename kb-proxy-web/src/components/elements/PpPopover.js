...
import frameDebounce from 'quasar-framework/src/utils/frame-debounce'
...
export default {
  ...
  props: {
    ...
    fit: Boolean,
    maxHeight: {type: String, default: '310px'},
  ...
  watch: {
    ...
    disable(v) {
      if (v) {
        this.anchorEl.classList.add('cursor-not-allowed');
        this.anchorEl.classList.remove('cursor-pointer');
      } else {
        this.anchorEl.classList.add('cursor-pointer');
        this.anchorEl.classList.remove('cursor-not-allowed');
      }
    }
  },
  computed: {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (this.anchorClick) {
        this.anchorEl.classList.add(this.disable ? 'cursor-not-allowed' : 'cursor-pointer');
  ...
  methods: {
    ...
    show(evt) {
      ...
      this.showPromise = new Promise((resolve, reject) => {
        this.showPromiseResolve = () => {
          ...
          this.$parent && this.$parent.$el && this.retire_class_list && this.$parent.$el.classList.remove(...this.retire_class_list);
          this.$parent && this.$parent.$el && this.active_class_list && this.$parent.$el.classList.add(...this.active_class_list);
          this.$emit('show', evt);
          resolve(evt)
        };
    ...
    __show(evt) {
      ...
      this.click_to_close && document.body.addEventListener('click', this.__bodyHide, true);
    ...
    hide(evt) {
      ...
      this.hidePromise = new Promise((resolve, reject) => {
        this.hidePromiseResolve = () => {
          ...
          this.$parent && this.$parent.$el && this.active_class_list && this.$parent.$el.classList.remove(...this.active_class_list);
          this.$parent && this.$parent.$el && this.retire_class_list && this.$parent.$el.classList.add(...this.retire_class_list);
          this.$emit('hide', evt);
          resolve();
        };
    ...
    __hide() {
      ...
      this.click_to_close && document.body.removeEventListener('click', this.__bodyHide, true);
